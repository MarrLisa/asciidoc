...
// используется в 3 РП, 8 РП, 18 РП, 20 РП, adding_experts, assign_experts, autocheck_pic, copy_del_app, creat_app_accreditation, creat_application, dialogue_registre, distribution_fund, list_applications, subsidy.adoc и др.
...
ifdef::manual3,manual18[]
С помощью поля «Выбрать профиль» можно менять профиль согласно профилям в учётной записи, как на <<id_image-g_mp_3, Рисунке {counter:figure-number}>>.
