...
class PDFConverterWithFullPageBorder < (Asciidoctor::Converter.for 'pdf')
  ...
  #   # @code = document.attr('code')
  # end

  # Тут правильно
  def init_pdf(doc)
    super(doc)
    @is_title_page = true # Флаг для проверки титульной страницы 
    @сontract_number = doc.attr('сontract_number') || 'Нет имени проекта'
    @code_doc = doc.attr('code') #|| 'Нет имени проекта'
    @version_doc = doc.attr('version') #|| 'Нет имени проекта'
    @page_all_doc = doc.attr('page_all')
    @ramka_doc = doc.attr('ramka')
    @year = doc.attr('year')
    @city = doc.attr('city')
  end
  ...
  def start_new_page(options = {})
    ...
    # Рисуем рамку только если это не первая страница
      # draw_full_page_title if page_number == 1

      draw_full_page_border if page_number > 1
    # end
  ...
  def draw_page_all
    ...
    go_to_page(1)
    canvas do
      line_width(1)                # Устанавливаем толщину линии
      # stroke_color '707070'         # Серый цвет для рамки
       # Серый цвет для текста 
      # Получаем размеры страницы
      page_width = page.dimensions[2]  # Ширина страницы
      page_height = page.dimensions[3] # Высота страницы
      mesto_list_x = page_width/2 - 15
      mesto_list_y = 85
      my_font_size = 14

      fill_color 'FFFFFF'
      fill_rectangle [mesto_list_x-26, mesto_list_y+15], 100, 50
      fill_color '333333'
      draw_text "Листов #{page_count}", at: [mesto_list_x, mesto_list_y], size: my_font_size
      draw_text "#{@city} #{@year}", at: [mesto_list_x-10, mesto_list_y-20], size: my_font_size
  ...
  def draw_full_page_border
    ...
    canvas do
      line_width(1)                # Устанавливаем толщину линии
      stroke_color '707070'         # Серый цвет для рамки
      # fill_color '707070' # Серый цвет для текста 
      # Получаем размеры страницы
      page_width = page.dimensions[2]  # Ширина страницы
      page_height = page.dimensions[3] # Высота страницы
      ...
      if @ramka_doc == '0'
        draw_text "#{page_number}", at: [page_width/2, 25], size: my_font_size
        draw_page_all
      else  
# **********************************
        if page_width < 600
        ...
        interval = 7
        otstup_text = 20
        list_mesto = page_width - otstup_right - w_table + w_table/5
        list_i_mesto = page_width - otstup_right - w_table + w_table/3
          ...
          font_families.update("Times" => {
            normal: "modules/2_stage_mf/resources/fonts/times-new-roman-normal.ttf",
            bold: "modules/2_stage_mf/resources/fonts/times-new-roman-bold.ttf",
            italic: "modules/2_stage_mf/resources/fonts/times-new-roman-italic.ttf",
            bold_italic: "modules/2_stage_mf/resources/fonts/times-new-roman-bold-italic.ttf"
          })
        


        font('Times', style: :normal) do # Задаем шрифт с жирным стилем
        # Рисуем внешний прямоугольник от верхнего левого угла до нижнего правого
        stroke_rectangle [otstup_left, page_height-otstup_up], page_width- otstup_left - otstup_right, page_height- otstup_down - otstup_up     
        
        # # Рисуем внутренний прямоугольник от верхнего левого угла до нижнего правого
        stroke_rectangle [otstup_left, h_down_table+otstup_down], page_width- otstup_left - otstup_right, h_down_table
        draw_text "Код программного средства – ", at: [otstup_left + otstup_text, otstup_down + 5*interval], size: my_font_size
        
        draw_text "Номер версии – #{@version_doc}", at: [otstup_left + otstup_text, otstup_down + 3*interval], size: my_font_size
        
        draw_text "Код документа – #{@code_doc}", at: [otstup_left + otstup_text, otstup_down + 1*interval], size: my_font_size
        
        # # Рисуем прямоугольник Для ЛИСТ и его число
        stroke_rectangle [page_width - otstup_right - w_table, h_down_table+otstup_down], w_table, h_down_table
        #       # Рисуем прямоугольник Для ЛИСТ маленький
        stroke_rectangle [page_width - otstup_right - w_table, h_down_table+otstup_down], w_table, h_down_table/2
        
        # пишем листы
        draw_text "Лист", at: [list_mesto, otstup_down + 5*interval], size: my_font_size
        draw_text "#{page_number}", at: [list_i_mesto, otstup_down + 1*interval], size: my_font_size
        
        draw_page_all
        puts "Выполняется: #{page_number}"
